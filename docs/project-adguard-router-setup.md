...
| Web UI | `127.0.0.1:8082` (proxied by Caddy at `addblock.veracoechea.com`) |
...
- Visit `https://addblock.veracoechea.com` - you should see the AdGuard onboarding wizard
...
1. **From a LAN device** (phone on WiFi, laptop on Ethernet):
   ...
   nslookup google.com
   ```
   Should resolve via `10.0.0.2` (AdGuard). Check the AdGuard query log at `https://addblock.veracoechea.com` - you should see the query appear.
...
3. **Check the AdGuard dashboard**
   - Go to `https://addblock.veracoechea.com`
...
1. Go to `https://addblock.veracoechea.com` > Filters > DNS blocklists
