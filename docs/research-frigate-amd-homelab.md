...
Scope: Frigate `0.17.2`, with no deployment selection. Frigate and ROCm claims below use version-pinned first-party sources. NixOS claims were checked with nix-mcp against the current unstable option index.
...
- There is no Frigate service, camera, RTSP/ONVIF, or MQTT configuration in this checkout. Camera credentials, inventory, storage requirements, and whether Home Assistant integration is needed are unknown.

## Deployment options

| Concern | Native NixOS | Declarative Podman OCI |
| --- | --- | --- |
| Service shape | `services.frigate.settings` accepts Frigate YAML as a Nix attribute set. Its `hostname` option configures an Nginx virtual host. [^nix-frigate-settings] [^nix-frigate-hostname] | `virtualisation.oci-containers` runs OCI containers as systemd services. Its backend defaults to Podman. [^nix-oci-backend] |
| Upstream release | The NixOS package choice must be verified against the flake pin at implementation time. | Frigate 0.17.2 documents the `-rocm` image for AMD object detection. The image Dockerfile installs ROCm 7.1.1 and its requirements pin ONNX Runtime MIGraphX 1.23.1. [^frigate-detector] [^frigate-dockerfile] [^frigate-requirements] |
| Hardware access | The module option documents AMD VA-API support when `hardware.graphics.enable` is enabled and `vaapiDriver = "radeonsi"` is used. [^nix-frigate-vaapi] | OCI containers can attach named host devices through `devices`. [^nix-oci-devices] |
| Secret boundary | Do not put camera or MQTT credentials in the Nix attribute set: this is configuration, not a secret boundary. | OCI `environmentFiles` loads absolute-path environment files. Frigate substitutes only supported `FRIGATE_*` variables in config values. [^nix-oci-environment-files] [^frigate-config] |
...
6. With the Frigate 0.17.2 `-rocm` image, test native GPU detection first. Only then test an `HSA_OVERRIDE_GFX_VERSION` value if needed. Frigate recommends first starting the ONNX detector with object detection disabled so model conversion can complete and cache, then enabling detection. [^frigate-detector]
7. Run a sustained camera plus Ollama test. Record detection latency, dropped frames, FFmpeg errors, GPU use, VRAM use, and restart recovery. The RX 7600 is shared with Ollama, so this gate must pass before either deployment path is selected.
8. Validate the final credential-free configuration with the exact Frigate image before deployment. Frigate exits when config validation fails and documents a container command that returns nonzero for an invalid configuration. [^frigate-advanced]
