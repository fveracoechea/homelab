...
- **WireGuard manual mesh** (rejected) — no embedded DERP or web UI for node management. Headscale gives us automatic NAT traversal and key rotation without the SaaS dependency.
...
- Headscale DNS management and MagicDNS are disabled. Clients use DNS from their current network. This lets LAN clients use AdGuard Home through DHCP and avoids a tailnet DNS dependency for public traffic.
