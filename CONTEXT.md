...
- **Baby-shower invitation app** — the public combined baby-shower and gender-reveal invitation running as a Nix-built OCI container on the hostinger VPS (`services/baby-shower.nix`). Bun builds the TanStack Start application; Node.js runs the Nitro server because its `better-sqlite3` driver is not compatible with Bun. The SQLite database persists under `/var/lib/baby-shower/`. Domain: `baby.veracoechea.com`. The app protects the Admin view with its password login.
...
- **Home Assistant** — the smart-home automation platform running on the homelab (`services/home-assistant.nix`). NixOS-native (`services.home-assistant`). Listens on `127.0.0.1:8123`; Caddy vhost at `home.veracoechea.com`. No pre-created secrets required - admin user, home location, timezone, and integration credentials are all set through the onboarding wizard in the web UI and stored in `/var/lib/hass/.storage/`. Nix manages only the base config: `default_config` (enables the standard integration bundle) and `http` (reverse proxy settings for Caddy via `use_x_forwarded_for` + `trusted_proxies`). All automations, integrations, scripts, and dashboards are managed through the web UI. Bluetooth is enabled on the host (`hardware.bluetooth.enable`) for BLE device discovery; requires a Bluetooth adapter (built-in or USB dongle). Integrations enabled via `extraComponents` include: BLE sensors (switchbot, govee, inkbird, bthome, xiaomi, qingping, airthings, ruuvitag, etc.), ESPHome, weather (met), TP-Link (Kasa + Tapo), network printers (ipp), and LG webOS TV (`webostv` - media player, notifications, WoL power-on; see `docs/project-webos-tv.md`).
- **LG webOS TV** — the LG smart-TV integration in Home Assistant (`webostv` component in `extraComponents`). Controls an LG TV running webOS 2.0+ over the local network (TCP 3000/3001 outbound; no firewall change needed). Auto-discovered via SSDP (bundled in `default_config`). Pairing key is generated at pairing time and stored in `/var/lib/hass/.storage/` - no pre-created secrets. Power-on uses `wake_on_lan.send_magic_packet` (the `wake_on_lan` component is also in `extraComponents`) wired to a "Device is requested to turn on" automation on the webostv device - required since HA 2025.11, otherwise the media_player shows `unavailable` when off. Full setup walkthrough in `docs/project-webos-tv.md`.
...
- **Tailnet DNS** — Headscale DNS management and MagicDNS are disabled. Tailnet clients use DNS from their current network. On the LAN, ASUS DHCP gives clients AdGuard Home (`10.0.0.2`); away from home, clients use their Wi-Fi or cellular resolver. Public `*.veracoechea.com` records resolve homelab services to the tailnet IP, so service access does not require MagicDNS.
- **AdGuard Home** — the LAN-only ad-blocking DNS server running on the homelab (`services/adguardhome.nix`). NixOS-native (`services.adguardhome`). DNS listens on `10.0.0.2:53`, with the firewall open only on `enp8s0`. Web UI on `127.0.0.1:8082`; Caddy vhost at `ad-blocker.veracoechea.com`. Upstream DNS via Cloudflare (`1.1.1.1`), fallback via Quad9 (`9.9.9.9`); bootstrap DNS (`1.1.1.1`, `9.9.9.9`) used only for resolving blocklist URLs on startup. Settings are managed by Nix because `mutableSettings = false`.
...
└── services/
    ...
    ├── baby-shower.nix            ← public invitation app on VPS (baby.veracoechea.com, OCI + SQLite)
...
- The Baby-shower invitation app source is pinned as the non-flake `baby-shower` input. Nix installs its locked dependencies with bun2nix, builds the Nitro output with Bun, and runs it with Node.js in a read-only OCI container. Drizzle applies migrations from `/app/drizzle` before the first database use. The app protects the Admin view with its password login.
- AdGuard Home settings are managed by Nix because `mutableSettings = false`. LAN devices use AdGuard through the router's only DHCP DNS setting (`10.0.0.2`). Headscale DNS management is disabled, so tailnet clients use DNS from their current network. Public `*.veracoechea.com` records resolve services to the homelab tailnet IP. The homelab itself does not accept tailnet DNS (`--accept-dns=false` in `services/tailscale.nix`) to avoid a DNS boot-order dependency.
